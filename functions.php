...
function themeprefix_slick_enqueue_scripts_styles() {
	wp_enqueue_script( 'slickjs', get_stylesheet_directory_uri() . '/js/slick.min.js', array( 'jquery' ), '1.0.0' );
...
//						'home'		=>	__('Home' , 'easy-property-listings'),
			'wentworth'		=>	__('Wentworth' , 'easy-property-listings'),
		)
	);
	return $fields;
}
...
add_action('wp_enqueue_scripts', 'themeprefix_slick_enqueue_scripts_styles' ); // slick carousel
...
function add_key_details_meta_box_epl_listings_callback($meta_fields) {
	$custom_field = array(
			'id'		=>	'epl-property-listing-key-details-id',
			'label'		=>	__('Extra Details', 'epl'),
			'post_type'	=>	array('property', 'rural', 'rental', 'land', 'commercial', 'commercial_land', 'business'),
			'context'	=>	'normal',
			'priority'	=>	'default',
			'groups'	=>	array(
				array(
					'id'		=>	'property_key_details_section',
					'columns'	=>	'3',
					'label'		=>	'Property key details',
					'fields'	=>	array(
						array(
							'name'		=>	'property_key_details_council_rates',
							'label'		=>	__('Council rates (per quarter)', 'epl'),
							'type'		=>	'decimal',
							'maxlength'	=>	'50'
                        ),
						array(
							'name'		=>	'property_key_details_council_water',
							'label'		=>	__('Council water rates (per quarter)', 'epl'),
							'type'		=>	'decimal',
							'maxlength'	=>	'50'
                        ),
                        array(
							'name'		=>	'property_key_details_strata_levies',
							'label'		=>	__('Strata levies (per quarter)', 'epl'),
							'type'		=>	'decimal',
							'maxlength'	=>	'50'
						)
					)
...
add_filter( 'epl_listing_meta_boxes' , 'add_key_details_meta_box_epl_listings_callback' , 1);


function property_key_details() {
	global $property;
	// Get the custom variable
    $council_rates	= $property->get_property_meta( 'property_key_details_council_rates' );
    $council_water	= $property->get_property_meta( 'property_key_details_council_water' );
    $strata_levies	= $property->get_property_meta( 'property_key_details_strata_levies' );
    

	if ( isset ($council_rates) ) {
        echo '<div class="custom-class">Council Rates $' . $council_rates . ' per quarter</div>';
    }
	if ( isset ($council_water) ) {
        echo '<div class="custom-class">Council Water $' . $council_water . ' per quarter</div>';
    }
	if ( isset ($strata_levies) ) {
        echo '<div class="custom-class">Strata Levies $' . $strata_levies . ' per quarter</div>';
    }
}
// This will echo the custom variable beside the price
add_action( 'wp_property_key_details' , 'property_key_details' );
...
add_action( 'epl_property_details' , 'property_details' );


/**
 * Property Inspection Times
 */
function wp_property_inspection_times() {
	global $property;
...
add_action( 'wp_property_inspection_times', 'wp_property_inspection_times' );
...
function rec_epl_search_widget_option_label_location() {
   ...
   add_filter( 'epl_search_widget_option_label_carport' , 'rec_epl_search_widget_option_label_parking' );
